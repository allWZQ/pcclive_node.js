  ...
  _bindInstallEvents: function () {
    $("#btn-submit").on("click", function (t) {
      ...
      var r = $("#dbname").val(),
        ...
        e = $("#username").val(),
        n = $("#pwd").val();
      return "string" != typeof r || 0 === r.length
        ? ($("#dbname").get(0).focus(),
        ...
        : ($("#btn-submit").text("正在安装……"),
          void $.ajax({
            url: "/install",
            type: "POST",
            data: { dbname: r, dbip: o, username: e, password: n },
            dataType: "json",
            success: function (t) {
              if (-1 === t.status) {
                if (t.msg.code)
                  return (
                    $("#btn-submit").text("立即安装"),
                    alert(
                      "出错了：连接数据库服务器失败，请检查MySQL数据库服务是否已经启动，或数据库主机配置是否正确！"
                    )
                  );
                if (0 < t.msg.indexOf("database"))
                  return (
                    $("#btn-submit").text("立即安装"),
                    alert("数据库名不存在（未知的数据库名）！")
                  );
                $("#btn-submit").text("立即安装"),
                  alert("出错了:" + t.msg + "\n(数据库用户名或密码不正确)！");
              } else if (1 === t.status)
                var r = 4,
                  o = setInterval(function () {
                    r--,
                      ...
                      0 === r &&
                        (clearInterval(o), (window.location.href = "/"));
                  }, 1e3);
            },
          }));
    });
